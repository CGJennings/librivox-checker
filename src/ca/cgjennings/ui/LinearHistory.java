...
/**
 * A <code>LinearHistory</code> captures a sequence of user actions
 * and allows them to be replayed, as with the forward/back operations
 * in a browsing interface.
 *
...
public class LinearHistory<P> {
	...
	private List<P> list = new LinkedList<P>();

	/**
	 * Creates a new linear history that tracks state objects
	 * (URL, location, document, etc.) of type <code>P</code>.
	 */
	...
	/**
	 * Go directly to a new position (without using forward or back) and
	 * display the position.
	 * Any positions after the current position in the history are deleted, and
	 * the new <code>position</code> becomes the end of the history.
	 *
	 * @param position
	 */
...
//		}
	...
	/**
	 * Called when a new position is visited
	 * (via {@link #go}, {@link #back}, or {@link #forward}).
	 * The base class does nothing; subclasses may override this to customize
	 * behaviour.
