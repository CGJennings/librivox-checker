 ...
 * of threads. The manager limits the number of jobs that are allowed to run
 * in parallel based on the number of CPUs, thus preventing the system from
 * being overwhelmed when a large number of files are added to the list of
 * files to check.
 *
 * <p><b>Note:</b> This package-private class is used internally
...
class JobManager {
	 ...
	 * the file, so two files can reasonably be worked on in parallel even
	 * if there is only 1 CPU.)
	 */
	private static final int DEBUG_THREAD_COUNT = -1;

	/**
	 * If <code>true</code>, then the threads created for the thread pool
	 * will request higher than normal priority. The mapping of the this
	 * request to a native thread priority on the host system is
	 * platform-dependent, so it may or may not have an effect on performance.
	 */
	private static boolean USE_HIGHER_PRIORITY_THREADS = false;

	/**
	 * The length of time, in ms, to keep a thread around waiting for a new
	 * job before allowing it to die.
	 */
	 ...
	 * manager by calling {@link #analyzeInFuture}. The token can be used
	 * to cancel a queued job before it starts running.
	 */
	public static class JobToken {
		 ...
		 * @return the file that, when submitted to the job manager,
		 *     produced this token
		 */
		...
		/**
		 * Cancels this job. If the job hasn't started, it will never be run.
		 * If it has started but hasn't finished, the thread's interrupt status
		...
		public void waitFor( int ms ) throws InterruptedException {
			try {
				if( !f.isCancelled() ) f.get( ms, TimeUnit.MILLISECONDS );
			} catch( ExecutionException e ) {
				Checker.getLogger().log( Level.SEVERE, "uncaught exception in analysis job", e );
