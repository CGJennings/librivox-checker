...
/**
 * Measures background noise by capturing the quietest part
 * of the audio.
 *
...
public class NoiseValidator extends AbstractValidator {
	...
	/**
	 * The smallest means of squares of the windows we have found so far.
	 * That is, we take the RMS amplitude of each window, and look for the
	 * one with the smallest value. The only difference is that we don't
	 * bother taking the square root since we don't care about the exact
	 * RMS value, just which one would be smallest.
	 */
	...
	public void beginAnalysis( AudioHeader header, Validator[] predecessors ) {
		...
		if( winSize < 0 ) throw new AssertionError();
	...
	/**
	 * The pointer to the position in the buffer where the next frame
	 * should be written
	 */
	private int p;
