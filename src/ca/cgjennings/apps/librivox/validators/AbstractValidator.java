...
/**
 * An abstract base class for {@link Validator}s.
 * This abstract base class simplifies the task of creating a validator
 * and obviates the need to deal directly with the validation {@link Report}.
 * <p>
 * This class provides facilities for a <code>Validator</code> to
 * obtain configuration parameters at runtime from settings files.
 * The base settings file for a given validator is located in the
 * <code>resources</code> folder and has the same name as the validator's
 * class name, with the <code>.properties</code> file extension.
 * Alternate versions of the settings file can be defined based on the
 * current validation strictness setting. Currently, the only valid strictness
 * levels are strict (no suffix) and gentle (settings defined in a file
 * that adds "_g" to the class name).
 * <p>
 * This base class uses the protected versus public attribute of methods
 * to suggest how they are intended to be used. Protected methods are
 * not normally used directly, but are accessed indirectly by higher-level
 * methods designed to simplify report generation.
 *
...
public abstract class AbstractValidator implements Validator {
	...
	/**
	 * You must override this method to return the general category
	 * of testing performed by this validator.
	 *
	...
	/**
	 * You must override this method to return <code>true</code> if you
	 * wish to process the file's audio data.
	 * {@inheritdoc }
	 * <p>
	...
	/**
	 * This method is overridden by the abstract base class in order
	 * to initialize the services the base class provides. It is recommended
	 * that validator writers not override this method, but instead do
	 * their initialization in {@link Validator#beginAnalysis}. If you
	 * do override this method, then you <b>must</b> call the super
	...
	/**
	 * Returns the report that this validator should write results to.
	 * Note that validators should not normally use this method directly,
	 * but should instead use the report-writing features provided by the
	 * base class.
	 *
	...
	/**
	 * Record the result of a test with rule name <code>rule</code> as a failure,
	 * using <code>value</code> to describe the result to the user.
	 * This will add an entry to the report's validation section describing
	 * the outcome.
	 * <p>
	 * Depending on the value of <code>ruleKey</code> and the class's
	 * strictness setting (as set in <code>validators.classlist</code>),
	 * the failure result may be downgraded to a warning.
	 *
	 * @param ruleKey the name of the rule's key in the validator's settings file
	...
	/**
	 * Record the result of a test with rule name <code>ruleKey</code> as a warning,
	 * using <code>value</code> to describe the result to the user.
	 * This will add an entry to the report's validation section describing
	 * the outcome.
	 *
	...
	/**
	 * Adds a key, value pair to the information part of the report.
	 * For example, the key might be <code>"Sampling Rate"</code> and
	 * the value might be <code>"44.1 kHz"</code>.
	 *
	 ...
	 * {@inheritdoc }
	 * <p>
	 * This base class implementation throws an <code>UnsupportedOperationException</code>.
	 * The exception will be thrown if {@link #isAudioProcessor} returns
	 * <code>true</code> and this method has not been overridden.
	 */
	@Override
	 ...
	 * {@link #updateValidity} can correctly downgrade the validity of a file
	 * as it fails tests.
	 *
	 ...
	 * <tt>resources/<i>classname</i>.properties</tt>, if it exists, or
	 * a locale-specific version of the file if that is available.
	 *
	 * @return a shared <code>Settings</code> instance with settings for
	 *         this validator
	 */
	 ...
	 * override the default settings by calling it once with an empty suffix
	 * and once with the suffix of the desired strictness.
	 */
	...
	/**
	 * Return a localized (and possibly formatted) text string.
	 * The value of <code>key</code> is first looked for in
	 * the localized settings for this class. If no value is found there,
	 * then the value is looked for in the application's
	 * <tt>text_<i>XX</i>.properties</tt> file. If no value for the
	 * key is found, a default string equal to
	 * <code>"[MISSING STRING " + key + "]"</code> is used.
	 * <p>
	 * If the optional <code>arguments</code> are provided,
	 * they will be used to format the fetched string as if by<br>
	 * <code>
	 ...
	 * <p>
	 * Subclasses should not normally call this directly, but should allow
	 * it to be set for them through calls to {@link #fail} and {@link #warn}.
	 *
	...
	/**
	 * Update the validity value with a new result. This method's validity will only
	 * be changed if <code>v</code> is "worse" than the current validity.
	...
	/**
	 * Returns a {@link Strictness} that represents an enforcement level
	 * for a validation test. The result is determined from the value of
	 * the setting returned by {@link #getSettings}<code>( key )</code>.
	 * If <code>key</code> is not defined, it will default to
	 * <code>REQUIRED</code>.
	 *
	...
	/**
	 * Applies a {@link Strictness} setting to a test {@link Validity}, downgrading
	 * <code>validity</code> if it is a stronger result than
	...
	/**
	 * This method updates the validity state of this validator and
	 * writes an appropriate message to the validation report. This consists
	 * of the following steps:
	 * <ol>
	 * <li> If <code>ruleKey</code> is not <code>null</code>, then an
	 *      enforcement rule will be used to modify the validity level.
	 *      The value of <code>ruleKey</code> will be looked up in the settings
	 *      for this method using {@link #getTestStrictness(java.lang.String)}.
	 ...
	 * </ol>
	 * <p>
	 * If the validator's settings defines a key equal to the rule name
	 * plus the suffix <code>"-help"</code>, then the report will attempt to
	 * use that as a help file that the user can consult about the rule.
	 * <p>
	 * (Note that the final validity for the validator will be still be determined by
	 * applying the validator class's <code>Strictness</code> setting.)
	 *
	 * @param category the category of the report that this result belongs in
	 * @param report the report to write this message to
	 * @param ruleKey the key to use to determine the test's strictness (may be <code>null</code>)
	...
	/**
	 * Returns the name of the validator as a human-friendly string.
	 * This is used to name the validator in the Choose Validators dialog.
