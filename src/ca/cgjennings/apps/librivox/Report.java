...
public class Report {
	...
	/**
	 * Add a feature description to the report. These are
	 * information entries that do not affect the file's validity.
	 * A feature description is a pair a values consisting of a feature name
	 * and the value of that feature. For example, "Number of Channels", "2".
	 *
	 * @param cat the general category that the information applies to
	 * @param name the name of the feature
	 * @param value the value of the feature in the file analyzed for this report
	 */
	 ...
	 * overall validity of the file and itself has a validity value. A file
	 * will initially pass, but the final value will be the worst of all validity
	 * values of all validation entries. So, a single failed validation
	 * entry will cause the entire report (file) to fail.
	 *
	 * @param cat the general category of validation entry
	 * @param method the validator that is making the entry (may be <code>null</code>)
	 * @param validity the validity level to associate with the entry
	 * @param description a human-readable description (see {@link #format(java.lang.Object)})
	 * @param helpURL an optional string that provides a URL offering additional information
	 */
	...
	private void validationEntry( int reportID, StringBuilder b, Validity validity, Object description, String helpURL ) {
		...
		switch( validity ) {
			case PASS:
				image = PASS_ICON; break;
			case WARN:
				++warnings;
				image = WARN_ICON; break;
			case FAIL:
			case INCOMPLETE:
				++errors;
				image = FAIL_ICON; break;
			default:
	...
	/**
	 * Mark the report as closed and prepare the final report text.
	 * Once a report is closed, no more entries may be added to it.
	 */
	public void close() {
		isOpen = false;
	...
	/**
	 * Used to format descriptions that are to be included in a report.
	 * May be <code>null</code>, in which case an empty string is returned.
	 * Otherwise, if the object is <code>Iterable</code>, a sequence of lines
	 * is returned by recursively <code>format</code>ting each element in the
	 * iterable, separating them with line break codes (<tt>&lt;br&gt;</tt>).
	 * If the object is not iterable, then its string representation is returned
	...
	/**
	 * Return the information report for this document, or <code>null</code>
	 * if the report has not been closed.
	 * @return
	 */
	...
	private void checkClosure() {
		if( !isOpen ) throw new IllegalStateException( "report is closed" );
	}

	/** Creates an array of <code>StringBuilders</code> (one for each <code>Category</code>). */
	...
	/**
	 * Return a document that can be used as a placeholder when a valid
	 * report is not selected.
	 *
	...
	/**
	 * Overrides the standard report content with an error message.
	 * If an error occurs in the system building the report, it can call this
	 * method to set an error message. When the report is closed, a
	 * report that consists of the error message will be generate instead of
	 * a standard report.
	 *
