...
/**
 * This is a skeletal implementation of a class that could potentially
 * be used to editable sections to reports. Currently, it is only used
 * to display the large version of the cover art thumbnail when the user
 * clicks this in the information report.
 * 
...
public class MetadataEditorLinkFactory {
	...
	public static void setEditorParentFrame( JFrame app ) {
		...
		if( parent != null ) throw new IllegalStateException( "parent already set" );
		parent = app;
	}
...
//			}
	...
	public static synchronized void unlink( LibriVoxAudioFile file ) {
		...
		if( linkText != null ) linkMap.remove( linkText );
	}
	...
	static DecodedLink decodeLink( String text ) {
		if( !isLink( text ) ) throw new IllegalArgumentException( "not a metadata link" );
		String[] tokens = text.substring(1).split("\\!");
		DecodedLink dl = new DecodedLink();
		dl.file = linkMap.get( "!" + tokens[0] );
		if( dl.file == null ) throw new IllegalStateException("file not in map");
