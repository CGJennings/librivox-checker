...
/**
 * A class that encapsulates basic metadata about an MP3 file. Most of the
 * data is obtained from ID3 tags, where available.
 *
...
public class MP3FileMetadata {
	/**
	 * Creates a new instance based on the metadata stored in the given
	 * file. The file must be writable, or an I/O exception will be thrown.
	 *
	...
	/**
	 * Returns the offset to the start of audio data within the file.
	 * If there is metadata at the start of the file, this will return
	 * how many bytes to skip before the audio begins.
	 *
	...
	/**
	 * Returns a view of the ID3 tag metadata. If version 2 is available,
	 * that will be returned. Otherwise, if version 1 is available, that
	 * will be returned. Otherwise, returns <code>null</code>.
	...
	/**
	 * Returns a view of the ID3 version 1 metadata in the file,
	...
	/**
	 * Returns a view of the ID3 version 2 metadata in the file,
	 * or <code>null</code> if it does not exist.
	 *
	...
	/**
	 * Call this method to re-read metadata from the source file
	...
	/**
	 * Create an MP3File for reading metadata from a file, or return <code>null</code>
	 *
	 * @param f
	 * @return
	 * @throws IOException
	...
	private void fillIn( MP3File mp3f ) {
		...
		if( startOfAudio < 0 ) startOfAudio = 0;
		trackLen = h.getPreciseTrackLength();
		long fcount = h.getNumberOfFramesEstimate();
		if( fcount > Integer.MAX_VALUE ) fcount = Integer.MAX_VALUE;
		frames = (int) fcount;
		encoder = h.getEncoder();
		if( encoder.length() == 0 ) encoder = null;
	...
	/**
	 * The tag processor tends to barf when a tag is absent.
	 * Rather than surround every attempt to get a tag with
	 * a try/catch, we use this generic interface.
	 *
	 * @param tag the metadata container
	 * @param method the name of the method to call for the desired type of metadata
