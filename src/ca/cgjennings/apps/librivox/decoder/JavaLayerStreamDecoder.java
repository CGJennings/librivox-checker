...
/**
 * An implementation of {@link StreamDecoder} that uses the
 * JavaLayer library to perform decoding.
 *
...
final class JavaLayerStreamDecoder implements StreamDecoder {
	 ...
	 * @param report a report that will be used to note any problems when decoding; may be <code>null</code>
	 * @param tolerance a hint regarding how many errors are acceptable before giving up
	 * @throws java.io.IOException if an I/O exception occurs while creating the decoder
	 */
	public JavaLayerStreamDecoder( InputStream in, Report report, ErrorTolerance tolerance ) throws IOException {
		this.report = report;

		switch( tolerance ) {
			case NONE: MAX_ERRORS = 0; break;
			case MODERATE: MAX_ERRORS = 10; break;
			case ALL: MAX_ERRORS = Integer.MAX_VALUE; break;
			default:
	...
	/**
	 * Return the number of valid frames that have been decoded by this
	 * decoder.
	 *
	 ...
	 * frames of audio. The decoder has not actually decoded the next frame,
	 * so it may turn out that the frame is corrupt and there are no more
	 * valid frames. The result of {@link #getNameFrame()} should always
	 * be checked.
	...
	/**
	 * Skips by the next frame of audio without decoding it.
	 * An {@link AudioFrame} is returned that contains frequency and
	 * channel information, but no sample data. (Although the sample
	 * buffer will be <code>null</code>, the sample count will indicate
	 * the number of samples that would have been present if the audio
	 * had been decoded.)
	 *
	...
	/**
	 * Return the next frame in the stream, or <code>null</code> if there are
	 * no more frames in the stream. Note that this method may return <code>null</code>
	 * even if {@link #mayHaveMoreFrames()} returns <code>true</code>.
	 *
	 * @return the next audio frame, or <code>null</code>
	 * @throws java.io.IOException if an error occurs and the decoder cannot recover
	 */
	@Override
	...
	private void handleDecodingError( JavaLayerException e, boolean logInReport ) throws IOException {
		...
		switch( code ) {
			case BitstreamErrors.INVALIDFRAME:
				key = "invalid-frame"; break;
			case BitstreamErrors.STREAM_ERROR:
				key = "stream"; isIOError = true; break;
	...
	/**
	 * A utility method that returns the implementation-indepedent
	 * channel format value for a given frame header.
	 ...
	 *   never happen in practice as decoding the frame should raise a
	 *   decoding exception
	 */
