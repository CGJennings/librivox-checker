...
/**
 * A frame of sequential audio samples in 16-bit PCM format.
 * An audio frame is a passive container for data generated by a decoder.
 * It insulates the rest of the application from the particular decoder
 * that is being used. Rather than create a new frame object as each frame
 * is read, a single instance is shared and updated after each frame
 * has been processed.
 *
 * <p>The sample buffer will also be shared across all of the
 * various {@link AnalysisMethod}s. Therefore, the contents of the
 * sample buffer should never be written to. If a mutable buffer
 * is required, clone the provided buffer and mutate the contents
 * of the clone.
 *
...
public class AudioFrame {
	...
	/**
	 * Creates a new, uninitialized audio frame container.
	 * Before passing the frame to a client, it must be initialized
	 * with appropriate data by calling {@link #set}.
	 */
	public AudioFrame() {
	}
	...
	/**
	 * Returns the number of channels in the audio frame; normally expected
	 * to be constant for an entire file.
	 *
	...
	/**
	 * Returns the number of samples for each channel that are available in
	 * the buffer.
