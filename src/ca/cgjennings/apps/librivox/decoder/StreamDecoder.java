 ...
 * one frame at a time. Although a "frame" here is usually equivalent to an
 * MP3 frame, it need not be.
 * <p>
...
public interface StreamDecoder {
	 ...
	 * channel information, but no sample data.
	 *
	 * @return an audio frame with only header information and no samples for the skipped frame
	 ...
	 * so it may turn out that the frame is corrupt or is metadata and there are
	 * no more valid frames. If this returns <code>false</code>, then there are
	...
	/**
	 * Returns the audio header for the first frame of the stream.
	 * The audio header contains metadata
	 * about the audio format such as number of channels, frequency, and so on.
	 * Individual {@link AudioFrame}s will also provide some of the same information,
	 * as some values (the bit rate in variable bit rate streams)
	 * can legally change from frame to frame.
	 *
	 * @return metadata about the stream based on sampling the headers of one or more frames of audio
	...
	public enum ErrorTolerance {
		...
		NONE,
		/** A hint that the decoder should tolerate a moderate number of errors before giving up. */
		MODERATE,
